...
export async function updateTicketStatus(input: UpdateTicketStatusInput) {
  ...
  const { data: ticket } = await supabase
    .from('tickets')
    .select('id, ticket_number, title, priority, requester_id, assigned_agent_id, description, category_id')
...
export async function escalateTicket(ticketId: string, currentLevel: number, assignToAgentId?: string) {
  ...
  const { data: ticket } = await supabase
    .from('tickets')
    .select('id, ticket_number, title, priority, requester_id, assigned_agent_id, created_at, description, category_id, locations(name, code)')
    .eq('id', ticketId)
    .single()
          ...
          <body style="margin:0; padding:0; background-color:#f9fafb;">
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background:#f9fafb; padding:40px 20px;">
              ...
              <div style="max-width:600px; margin:0 auto 24px auto; text-align:center;">
                <img src="https://ziii.com.mx/logos/1ZIIIlogo.png" alt="ZIII Helpdesk" width="180" height="120" style="display:block; margin:0 auto;" />
              </div>

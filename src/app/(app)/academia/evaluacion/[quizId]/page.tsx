...
export default function QuizPage() {
    ...
    try {
      ...
      if (!quizRes.ok) {
        if (quizRes.status === 404) {
          setError('Evaluaci+好 no encontrada');
          return;
        }
        throw new Error('Error al cargar la evaluaci+好');
      }
  ...
  const startQuiz = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'Error al iniciar la evaluaci+好');
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'Error al enviar la evaluaci+好');
      }
  ...
  if (error || !quiz) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-gray-500">
        ...
        <p className="text-lg">{error || 'Evaluaci+好 no encontrada'}</p>
        <button
          onClick={() => router.back()}
          ...
          {passedBefore && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
              <p className="text-green-700 flex items-center justify-center gap-2">
                ...
                Ya aprobaste esta evaluaci+好 anteriormente
              </p>
            </div>
          )}
          ...
          >
            {submitting ? (
              ...
              <>
                ...
                {previousAttempts.length > 0 ? 'Intentar de nuevo' : 'Comenzar evaluaci+好'}
              </>
            )}
          </button>
        </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <div className="bg-white rounded-xl shadow-sm border p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            {passed ? '-檦elicidades!' : 'No aprobaste'}
          </h1>
          <p className="text-gray-600 mb-6">
            {passed
              ? 'Has aprobado la evaluaci+好 exitosamente.'
          ...
          <div className="flex justify-center gap-4">
            {quiz.module?.course_id && (
              <button
                onClick={() => router.push(`/academia/curso/${quiz.module?.course_id}`)}
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      <div className="mt-8 bg-gray-50 rounded-lg p-4">
        <p className="text-sm font-medium text-gray-700 mb-3">Navegaci+好 r+甑ida</p>

...
import { formatFileSize, uploadMaintenanceTicketAttachment, validateFile } from '@/lib/storage/attachments'
...
export default function MaintenanceTicketCreateFormModern({
  ...
  useEffect(() => {
    setDraftTicketId(`MANT-${Math.floor(Math.random() * 8999) + 1000}`)
  }, [])
  ...
  useEffect(() => {
    async function loadUser() {
      ...
      const allowed =
        profile &&
        (profile.role === 'admin' ||
          (['agent_l1', 'agent_l2', 'supervisor'].includes(profile.role) && profile.asset_category === 'MAINTENANCE'))
  ...
  useEffect(() => {
    async function loadAssets() {
      ...
      let query = supabase
        ...
        .select(
          'id, asset_code, category, brand, model, status, assigned_to_user_id, location_id, asset_location:locations!location_id(code, name)'
        )
        .is('deleted_at', null)
      ...
      if (assetsError) {
        console.error('[MaintenanceTicketCreateFormModern] Error cargando assets:', assetsError)
        setAssets([])
        return
      }

      const normalized = (assetsData ?? []).map((asset: any) => ({
        id: asset.id,
        ...
        model: asset.model,
        status: asset.status,
        assigned_to: asset.assigned_to_user_id,
    ...
    loadAssets()
  }, [canCreateForOthers, currentUserId, requesterId, supabase])
  ...
  async function createCategory() {
    ...
    try {
      ...
      if (categoryModalLevel === 2) {
        parentId = categoryL1 || null
      } else if (categoryModalLevel === 3) {
        parentId = categoryL2 || null
      }
      ...
      if (allCategories) {
        ...
        if (newCategory) {
          if (categoryModalLevel === 1) {
            setCategoryL1(newCategory.id)
            setCategoryL2('')
            setCategoryL3('')
          } else if (categoryModalLevel === 2) {
            setCategoryL2(newCategory.id)
            setCategoryL3('')
          } else if (categoryModalLevel === 3) {
            setCategoryL3(newCategory.id)
          }
        }
      }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const input = {
        title,
        description,
        service_area: 'maintenance' as const,
      ...
      const result = await createMaintenanceTicket(input)
      if (result?.error) {
      ...
      if (attachments.length > 0) {
        const userId = (canCreateForOthers && requesterId) || currentUserId
        if (userId) {
          const uploadResults = await Promise.all(
            attachments.map((file) => uploadMaintenanceTicketAttachment(ticketId, file, userId))
          )
          const failed = uploadResults.filter((r) => !r.success)
          if (failed.length > 0) {
            console.error('[MaintenanceTicketCreateFormModern] Algunos adjuntos fallaron:', failed)
          }
        }
      }

      router.push(`/mantenimiento/tickets/${ticketId}?created=1`)
      router.refresh()
    } finally {
      setBusy(false)
    }
  ...
  return (
    <div className="h-dvh overflow-hidden bg-slate-50 text-slate-800 font-sans selection:bg-orange-100 selection:text-orange-900 flex justify-center p-4 md:p-6 lg:p-8">
        ...
        <aside className="w-full lg:w-auto flex flex-col gap-4 flex-shrink-0">
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200/60">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 p-[2px]">
                <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
                  <span className="font-bold text-orange-600 text-sm">{initialsFromName(displayName)}</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 flex gap-3 items-start">
                <Activity size={14} className="text-orange-500 mt-0.5" />
                <div>
                  <p className="text-xs font-semibold text-slate-700">Consejo</p>
                  <p className="text-[10px] text-slate-500 leading-tight mt-0.5">
                    Adjunta fotos / evidencia del área afectada para acelerar la atención.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-900 rounded-xl p-5 shadow-lg text-white relative overflow-hidden hidden lg:block flex-1 min-h-[200px]">
            ...
            <div className="absolute bottom-0 left-0 w-24 h-24 bg-amber-500/20 rounded-full blur-2xl -ml-5 -mb-5" />

            <HelpCircle size={28} className="mb-4 text-orange-300" />
            <h4 className="font-bold text-base mb-2">Protocolo de Mantenimiento</h4>
            <ul className="text-xs text-slate-300 space-y-2 mb-4 leading-relaxed marker:text-orange-400 list-disc pl-4">
              <li>Indica ubicación exacta y referencia.</li>
              <li>Adjunta fotos si aplica.</li>
              <li>CRÍTICA solo si hay riesgo/seguridad.</li>
            </ul>
            <button
              type="button"
        ...
        <main className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200/60 overflow-hidden flex flex-col h-full min-h-0">
          <header className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-20">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-orange-50 rounded-lg border border-orange-100 flex items-center justify-center text-orange-600 shadow-sm">
                <FileText size={20} />
              </div>
              <div>
                <h1 className="text-lg font-bold text-slate-900 tracking-tight">Nueva Solicitud de Mantenimiento</h1>
          ...
          <div className="flex-1 min-h-0 overflow-auto p-5 lg:p-6 bg-slate-50/30">
            <form id="maintenance-ticket-create-form" onSubmit={onSubmit} className="h-full min-h-0 flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-3 space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:border-orange-500 focus:ring-orange-500/10 focus:ring-4 transition-all outline-none font-medium"
                      required
                      value={title}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-orange-600 hover:text-orange-700 hover:bg-orange-50 rounded-lg transition-colors"
                      >
                        <span className="flex items-center justify-center w-4 h-4 rounded-full bg-orange-100 text-orange-600 text-[10px] font-bold">+</span>
                        Nueva
                      </button>
                    </div>
                    <div className="relative">
                      <select
                        className="w-full px-4 py-3 bg-white border border-slate-200 rounded-lg text-sm focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none appearance-none cursor-pointer font-medium text-slate-700 shadow-sm"
                        value={categoryL1}
                        onChange={(e) => {
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        disabled={!categoryL1}
                        className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold text-orange-600 hover:text-orange-700 hover:bg-orange-50 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                        <span className="flex items-center justify-center w-4 h-4 rounded-full bg-orange-100 text-orange-600 text-[10px] font-bold">+</span>
                        Nueva
                      </button>
                    ...
                    <div className="relative">
                      <select
                        className="w-full px-4 py-3 bg-white border border-slate-200 rounded-lg text-sm focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none appearance-none cursor-pointer font-medium text-slate-700 shadow-sm disabled:bg-slate-100 disabled:text-slate-400"
                        value={categoryL2}
                        onChange={(e) => {
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <section className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200/60 shadow-sm">
                  ...
                  <div className="space-y-6">
                    <div>
                        ...
                        {(
                          ...
                          <button
                            ...
                            className={`relative px-2 py-3 rounded-lg border text-xs font-bold uppercase transition-all duration-200 flex flex-col items-center justify-center gap-1 ${
                              priorityPreset === p.id
                                ? `${p.color} ring-2 ring-offset-2 ring-offset-white ring-orange-200`
                      ...
                      {priorityPreset === 'critica' && (
                        <div className="mt-3 flex items-start gap-2 text-red-600 bg-red-50 p-3 rounded-lg text-xs">
                          ...
                          <p>
                            <strong>Atención:</strong> Prioridad CRÍTICA solo si hay riesgo o afectación severa.
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        className="w-full min-h-[160px] lg:min-h-[180px] px-4 py-4 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none resize-none font-normal leading-relaxed"
                        placeholder={'1. Ubicación exacta / referencia.\n2. ¿Qué sucede?\n3. ¿Desde cuándo?\n4. Fotos si aplica.'}
                        value={description}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Activo (opcional)</label>
                      <div className="relative">
                        <select
                ...
                <section className="lg:col-span-1 bg-white p-6 rounded-xl border border-slate-200/60 shadow-sm">
                  ...
                  <div
                    className="min-h-[160px] border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 hover:bg-orange-50/30 hover:border-orange-400 transition-all p-4 flex flex-col items-center justify-center text-center cursor-pointer relative group"
                    onDragOver={(e) => {
                      e.preventDefault()
                      e.stopPropagation()
                    }}
                  ...
                  >
                    ...
                    />
                    <div className="w-14 h-14 bg-white rounded-full shadow-sm border border-slate-200 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:border-orange-200 transition-all">
                      <Paperclip size={24} className="text-slate-400 group-hover:text-orange-600" />
                    </div>
                    <p className="text-sm font-bold text-slate-700 group-hover:text-orange-700 transition-colors">Subir Archivos</p>
                  ...
                  <div className="mt-4 space-y-2 overflow-y-auto max-h-[220px] pr-1">
                    {attachments.length === 0 && (
                      <div className="text-center py-4 text-xs text-slate-400 italic">Sin archivos adjuntos</div>
                    )}
                    {attachments.map((file, idx) => (
                      <div
                        key={`${file.name}-${idx}`}
                        className="flex items-center gap-3 p-2.5 bg-slate-50 border border-slate-200 rounded-lg group hover:border-orange-200 transition-colors"
                      >
                        <div className="w-8 h-8 bg-white rounded border border-slate-200 flex items-center justify-center text-orange-600 flex-shrink-0">
                          <FileText size={14} />
                        </div>
          ...
          <div className="px-8 py-5 border-t border-slate-100 bg-white flex justify-between items-center z-20">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <button
                type="submit"
                form="maintenance-ticket-create-form"
                disabled={busy}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-lg text-sm focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none"
                  autoFocus
                />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
              >

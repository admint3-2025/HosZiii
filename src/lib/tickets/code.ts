 ...
 * Formato: MANT-AAAAMMDD-XXXX
 */
export function formatMaintenanceTicketCode(params: { ticket_number: number; created_at: string | null }): string {
  const { ticket_number, created_at } = params
  ...
  if (!created_at) {
    ...
    const month = String(now.getMonth() + 1).padStart(2, '0')
    const day = String(now.getDate()).padStart(2, '0')
    return `MANT-${year}${month}${day}-${seq}`
  }

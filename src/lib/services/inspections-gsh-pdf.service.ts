...
export class InspectionGSHPDFGenerator {
  ...
  private async loadLogo(): Promise<void> {
    ...
    try {
      ...
      const { dataUrl, format } = await this.fetchImageAsDataUrl(InspectionGSHPDFGenerator.LOGO_URL)
      this.logoDataUrl = dataUrl
      this.logoFormat = format
      return
    } catch {
      // Fallback: proxy same-origin para evitar bloqueos CORS
      const proxyUrl = `/api/proxy-image?url=${encodeURIComponent(InspectionGSHPDFGenerator.LOGO_URL)}`
      try {
  ...
  private async loadBrandLogo(): Promise<void> {
    ...
    try {
      const { dataUrl, format } = await this.fetchImageAsDataUrl(InspectionGSHPDFGenerator.BRAND_LOGO_URL)
      this.brandLogoDataUrl = dataUrl
      this.brandLogoFormat = format
      return
    } catch {
      const proxyUrl = `/api/proxy-image?url=${encodeURIComponent(InspectionGSHPDFGenerator.BRAND_LOGO_URL)}`
      try {
        const { dataUrl, format } = await this.fetchImageAsDataUrl(proxyUrl)
        this.brandLogoDataUrl = dataUrl
        this.brandLogoFormat = format

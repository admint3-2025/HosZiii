-- ============================================================================
-- ADD MAINTENANCE CATEGORIES - COMPREHENSIVE HOTEL MAINTENANCE SYSTEM
-- ============================================================================
-- Propósito: Crear estructura COMPLETA de 3 niveles para tickets de 
--            mantenimiento general de hoteles
-- Estas categorías son independientes de las categorías IT
-- Total esperado: 15 Nivel 1 + ~200 Nivel 2
-- ============================================================================
...
-- PASO 2: CREAR CATEGORÍAS NIVEL 2 (SUBCATEGORÍAS) PARA CADA PADRE

-- 2.1 Subcategorías de CLIMATIZACIÓN / HVAC
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Climatización / HVAC' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.2 Subcategorías de FONTANERÍA / PLOMERÍA
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Fontanería / Plomería' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.3 Subcategorías de ELECTRICIDAD / ILUMINACIÓN
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Electricidad / Iluminación' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.4 Subcategorías de CARPINTERÍA / ESTRUCTURAS
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Carpintería / Estructuras' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.5 Subcategorías de PINTURA / ACABADOS
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Pintura / Acabados' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.6 Subcategorías de MOBILIARIO / DECORACIÓN
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Mobiliario / Decoración' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.7 Subcategorías de EQUIPOS DE COCINA
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Equipos de Cocina' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.8 Subcategorías de EQUIPOS DE LAVANDERÍA
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Equipos de Lavandería' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.9 Subcategorías de SISTEMAS DE SEGURIDAD
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Sistemas de Seguridad' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.10 Subcategorías de PISOS / REVESTIMIENTOS
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Pisos / Revestimientos' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.11 Subcategorías de ASCENSORES / ESCALERAS
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Ascensores / Escaleras' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.12 Subcategorías de SISTEMAS DE AGUA
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Sistemas de Agua' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.13 Subcategorías de DETECCIÓN DE INCENDIOS / SEGURIDAD
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Detección de Incendios / Seguridad' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.14 Subcategorías de MANTENIMIENTO GENERAL
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Mantenimiento General' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- 2.15 Subcategorías de EXTERIORES / ÁREAS COMUNES
INSERT INTO categories (name, slug, parent_id, sort_order)
SELECT v.name, v.slug, c.id, v.sort_order FROM (
...
CROSS JOIN (SELECT id FROM categories WHERE name = 'Exteriores / Áreas Comunes' AND parent_id IS NULL LIMIT 1) c
ON CONFLICT (parent_id, name) DO NOTHING;

-- PASO 3: VERIFICACIÓN
-- Contar categorías creadas
SELECT '✅ CATEGORÍAS DE MANTENIMIENTO CREADAS EXITOSAMENTE' as resultado;

-- Total de categorías por nivel
SELECT 
  COUNT(CASE WHEN parent_id IS NULL THEN 1 END) as "Nivel 1 (Raíz)",
  COUNT(CASE WHEN parent_id IS NOT NULL THEN 1 END) as "Nivel 2 (Subcategorías)",
  COUNT(*) as "Total"
FROM categories 
